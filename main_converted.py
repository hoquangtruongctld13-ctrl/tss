...
from tkinter import filedialog, messagebox, ttk as tkttk
...
# Modern UI with ttkbootstrap - no need to set theme here, it's set in Window init
...
class StudioGUI(ttk.Window):
    """Modern TTS Studio with ttkbootstrap - cleaner, more organized interface"""
    ...
    def __init__(self):
        # Initialize with modern darkly theme (Bootstrap-style dark theme)
        super().__init__(themename="darkly")

        # Window Setup
        self.title("VN TTS Studio - Modern Interface")
        
        # Set window size and center it properly
        window_width = 1400
        window_height = 900
    ...
    def _setup_ui(self):
        # Main Notebook (tabbed interface) - using ttkbootstrap
        self.notebook = ttk.Notebook(self)
        self.notebook.pack(fill="both", expand=True, padx=10, pady=10)

        # Create Tab Frames (simplified names, no icons as requested)
        self.tab_dashboard = ttk.Frame(self.notebook, padding=10)
        self.tab_longtext = ttk.Frame(self.notebook, padding=10)
        self.tab_multivoice = ttk.Frame(self.notebook, padding=10)
        self.tab_capcut = ttk.Frame(self.notebook, padding=10)
        self.tab_edge = ttk.Frame(self.notebook, padding=10)
        self.tab_vieneu = ttk.Frame(self.notebook, padding=10)
        self.tab_script = ttk.Frame(self.notebook, padding=10)
        self.tab_settings = ttk.Frame(self.notebook, padding=10)
        
        # Add tabs to notebook (text only, no icons)
        self.notebook.add(self.tab_dashboard, text="Gemini TTS")
        self.notebook.add(self.tab_longtext, text="Long Text Engine")
        self.notebook.add(self.tab_multivoice, text="Multi Voice")
        self.notebook.add(self.tab_capcut, text="Capcut Voice")
        self.notebook.add(self.tab_edge, text="Edge TTS")
        self.notebook.add(self.tab_vieneu, text="VN TTS (VieNeu)")
        self.notebook.add(self.tab_script, text="Script Reader")
    ...
    def _setup_settings_tab(self):
        ...
        google_frame = ttk.Frame(tab, bootstyle="primary")
        google_frame.grid(row=0, column=0, sticky="nsew", padx=5, pady=10)
        
        # Header
        header_google = ttk.Frame(google_frame, bootstyle="primary", height=40)
        header_google.pack(fill="x")
        ttk.Label(header_google, text="GOOGLE GEMINI TTS", font=("Roboto", 14, "bold"), foreground="white").pack(pady=8)

        # API Keys
        ttk.Label(google_frame, text="API KEYS (mỗi key 1 dòng)", font=("Roboto", 12, "bold"), foreground="#60a5fa").pack(anchor="w", padx=15, pady=(15, 5))
        ttk.Label(google_frame, text="Tối đa 5 key, tránh bị Google khoá", foreground="gray", font=("Arial", 10)).pack(anchor="w", padx=15)
        
        self.txt_api_keys = ScrolledText(google_frame, height=100, autohide=True, font=("Consolas", 11))
        ...
        ttk.Label(google_frame, text="System Instruction", font=("Roboto", 12, "bold"), foreground="#60a5fa").pack(anchor="w", padx=15, pady=(10, 5))
        self.txt_sys_instr = ScrolledText(google_frame, height=60, autohide=True, font=("Roboto", 11))
        self.txt_sys_instr.pack(fill="x", padx=15, pady=5)
        self.txt_sys_instr.insert("1.0", self.sys_instr_content)

        # Model parameters
        ttk.Label(google_frame, text="Model Parameters", font=("Roboto", 12, "bold"), foreground="#60a5fa").pack(anchor="w", padx=15, pady=(10, 5))
        
        # Grid nhỏ để chứa các switch cho gọn
        switch_frame = ttk.Frame(google_frame, bootstyle="transparent")
        switch_frame.pack(fill="x", padx=15)
        
        self.sw_thinking = ttk.Checkbutton(switch_frame, text="Thinking Mode")
        self.sw_thinking.pack(anchor="w", pady=2)

        self.sw_affective = ttk.Checkbutton(switch_frame, text="Biểu Cảm")
        self.sw_affective.pack(anchor="w", pady=2)
        
        self.sw_proactive = ttk.Checkbutton(switch_frame, text="Proactive Audio")
        self.sw_proactive.pack(anchor="w", pady=2)

        self.lbl_budget = ttk.Label(google_frame, text="Thinking Budget: 1024")
        ...
        self.slider_budget = ttk.Scale(google_frame, from_=0, to=4096, number_of_steps=32, command=update_budget)
        ...
        ttk.Frame(google_frame, height=2, bootstyle="primary").pack(fill="x", padx=15, pady=10) # Divider
        
        ttk.Label(google_frame, text="Multi-Threading (Tốc độ)", font=("Roboto", 12, "bold"), foreground="#a78bfa").pack(anchor="w", padx=15, pady=(5, 5))
        
        self.sw_multi_worker = ttk.Checkbutton(google_frame, text="Bật đa luồng (Nhanh hơn)")
        self.sw_multi_worker.pack(anchor="w", padx=15, pady=5)
        if self.multi_worker_enabled_val: self.sw_multi_worker.select()

        self.lbl_workers = ttk.Label(google_frame, text=f"Workers per Key: {self.workers_per_key_val}")
        ...
        self.slider_workers = ttk.Scale(google_frame, from_=1, to=50, number_of_steps=49, command=update_worker_lbl)
        ...
        capcut_frame = ttk.Frame(tab, bootstyle="primary")
        capcut_frame.grid(row=0, column=1, sticky="nsew", padx=5, pady=10)
        
        header_capcut = ttk.Frame(capcut_frame, bootstyle="primary", height=40)
        header_capcut.pack(fill="x")
        ttk.Label(header_capcut, text="CAPCUT VOICE", font=("Roboto", 14, "bold"), foreground="white").pack(pady=8)

        ttk.Label(capcut_frame, text="Session ID (TikTok)", font=("Roboto", 12, "bold"), foreground="#f87171").pack(anchor="w", padx=15, pady=(15, 5))
        ttk.Label(capcut_frame, text="Lấy từ Cookie sessionid của TikTok", foreground="gray", font=("Arial", 10)).pack(anchor="w", padx=15)
        
        self.entry_capcut_session = ttk.Entry(capcut_frame, placeholder_text="Nhập Session ID...", font=("Consolas", 11))
        ...
        ttk.Label(capcut_frame, text="Thêm Voice ID Mới", font=("Roboto", 12, "bold"), foreground="#f87171").pack(anchor="w", padx=15, pady=(20, 5))
        
        add_voice_frame = ttk.Frame(capcut_frame, bootstyle="transparent")
        add_voice_frame.pack(fill="x", padx=15, pady=5)
        
        self.entry_new_voice_id = ttk.Entry(add_voice_frame, placeholder_text="Voice ID (vd: BV074_streaming)", width=200)
        self.entry_new_voice_id.pack(side="left", padx=(0, 5))
        
        self.entry_new_voice_name = ttk.Entry(add_voice_frame, placeholder_text="Tên hiển thị", width=150)
        self.entry_new_voice_name.pack(side="left", padx=(0, 5))
        
        btn_add_voice = ttk.Button(add_voice_frame, text="+ Thêm", width=70, bootstyle="primary", command=self._add_custom_capcut_voice)
        btn_add_voice.pack(side="left")

        # Custom voices list
        ttk.Label(capcut_frame, text="Voice ID Tùy Chỉnh", font=("Roboto", 11), foreground="gray").pack(anchor="w", padx=15, pady=(15, 5))
        
        self.capcut_custom_list = ScrolledFrame(capcut_frame, height=150, bootstyle="primary")
        ...
        edge_frame = ttk.Frame(tab, bootstyle="primary")
        edge_frame.grid(row=0, column=2, sticky="nsew", padx=5, pady=10)
        
        # Header (Màu tím hoặc xanh đậm cho Edge)
        header_edge = ttk.Frame(edge_frame, bootstyle="primary", height=40)
        header_edge.pack(fill="x")
        ttk.Label(header_edge, text="EDGE TTS & FFMPEG", font=("Roboto", 14, "bold"), foreground="white").pack(pady=8)

        # FFmpeg Settings Section
        ttk.Label(edge_frame, text="FFmpeg Path", font=("Roboto", 12, "bold"), foreground="#a78bfa").pack(anchor="w", padx=15, pady=(15, 5))
        ttk.Label(edge_frame, text="Đường dẫn đến ffmpeg.exe (dùng cho ghép audio)", foreground="gray", font=("Arial", 10)).pack(anchor="w", padx=15)
        
        ffmpeg_row = ttk.Frame(edge_frame, bootstyle="transparent")
        ffmpeg_row.pack(fill="x", padx=15, pady=5)
        
        self.entry_ffmpeg_path = ttk.Entry(ffmpeg_row, placeholder_text="Đường dẫn ffmpeg.exe...")
        self.entry_ffmpeg_path.pack(side="left", fill="x", expand=True, padx=(0, 5))
        self.entry_ffmpeg_path.insert(0, self.ffmpeg_path)
        
        btn_browse_ffmpeg = ttk.Button(ffmpeg_row, text="📂", width=40, command=self._browse_ffmpeg)
        btn_browse_ffmpeg.pack(side="right")

        # Edge TTS Cache info
        ttk.Label(edge_frame, text="Edge TTS Data", font=("Roboto", 12, "bold"), foreground="#a78bfa").pack(anchor="w", padx=15, pady=(20, 5))
        ttk.Label(edge_frame, text="Dữ liệu giọng đọc từ Microsoft Edge", foreground="gray", font=("Arial", 10)).pack(anchor="w", padx=15)
        
        self.lbl_edge_cache = ttk.Label(edge_frame, text="Voice cache: Chưa tải", foreground="gray")
        self.lbl_edge_cache.pack(anchor="w", padx=15, pady=(10, 5))
        
        btn_refresh_edge = ttk.Button(edge_frame, text="Tải lại Voice List", bootstyle="primary", command=self._refresh_edge_voices)
        btn_refresh_edge.pack(fill="x", padx=15, pady=5)
        
        # Spacer để đẩy nút Save xuống đáy
        ttk.Frame(edge_frame, bootstyle="transparent").pack(fill="both", expand=True)

        # Save Button Area
        btn_save = ttk.Button(edge_frame, text="LƯU TẤT CẢ CÀI ĐẶT", height=50, 
                                 bootstyle="primary",  font=("Roboto", 14, "bold"),
    ...
    def _setup_dashboard_tab(self):
        ...
        main_paned = ttk.Frame(tab, bootstyle="transparent")
        main_paned.pack(fill="both", expand=True)
        
        # --- LEFT SIDEBAR ---
        left_col = ttk.Frame(main_paned, width=400, )
        left_col.pack(side="left", fill="y", padx=(0, 10), pady=5)
        
        # 1. File Input
        ttk.Label(left_col, text="FILE IO", font=("Roboto", 14, "bold"), foreground="#3B8ED0").pack(anchor="w", padx=15, pady=(15, 5))
        
        self.entry_file = ttk.Entry(left_col, placeholder_text="Chọn SRT/TXT file...")
        self.entry_file.pack(fill="x", padx=15, pady=5)
        
        btn_browse_in = ttk.Button(left_col, text="Browse Input", command=self._browse_input, bootstyle="primary", )
        btn_browse_in.pack(fill="x", padx=15, pady=2)

        self.entry_out = ttk.Entry(left_col, placeholder_text="./tts_output")
        self.entry_out.insert(0, "./tts_output")
        self.entry_out.pack(fill="x", padx=15, pady=(15, 5))
        
        btn_browse_out = ttk.Button(left_col, text="Browse Output", command=self._browse_output, bootstyle="primary", )
        btn_browse_out.pack(fill="x", padx=15, pady=2)

        self.entry_prefix = ttk.Entry(left_col, placeholder_text="File prefix (e.g. audio)")
        self.entry_prefix.insert(0, "audio")
        self.entry_prefix.pack(fill="x", padx=15, pady=(15, 5))

        # 2. Voice Config
        ttk.Label(left_col, text="VOICE CONFIG", font=("Roboto", 14, "bold"), foreground="#3B8ED0").pack(anchor="w", padx=15, pady=(25, 5))
        
        self.combo_voice = ttk.Combobox(left_col, values=VOICES)
        self.combo_voice.set(self.voice_val)
        self.combo_voice.pack(fill="x", padx=15, pady=5)

        self.combo_res = ttk.Combobox(left_col, values=list(MEDIA_RESOLUTIONS.keys()))
        self.combo_res.set("Medium (258 tokens/image)")
        self.combo_res.pack(fill="x", padx=15, pady=5)

        lbl_speed = ttk.Label(left_col, text="Speed: 1.0x")
        lbl_speed.pack(anchor="w", padx=15, pady=(10, 0))
        
        def update_speed(val):
            lbl_speed.configure(text=f"Speed: {float(val):.1f}x")
            
        self.slider_speed = ttk.Scale(left_col, from_=0.5, to=2.0, command=update_speed)
        ...
        self.tts_sw_live_session = ttk.Checkbutton(left_col, text="Live Session Mode")
        ...
        self.tts_sw_keep_voice_beta = ttk.Checkbutton(left_col, text="Giữ giọng beta")
        self.tts_sw_keep_voice_beta.pack(anchor="w", padx=15, pady=(5, 5))
        
        # Chunk settings for file processing (txt, docs, etc.)
        ttk.Label(left_col, text="CHUNK CONFIG (File dịch)", font=("Roboto", 12, "bold"), foreground="#3B8ED0").pack(anchor="w", padx=15, pady=(15, 5))
        
        chunk_frame = ttk.Frame(left_col, bootstyle="transparent")
        chunk_frame.pack(fill="x", padx=15, pady=5)
        
        ttk.Label(chunk_frame, text="Chunk size:", font=("Roboto", 10)).pack(side="left")
        self.gemini_chunk_size_entry = ttk.Entry(chunk_frame, placeholder_text="300", width=70)
        ...
        self.gemini_sw_chunk_v2 = ttk.Checkbutton(left_col, text="Ngắt dòng v2 (theo dấu câu)")
        self.gemini_sw_chunk_v2.pack(anchor="w", padx=15, pady=5)
        if self.gemini_chunk_v2_enabled:
            self.gemini_sw_chunk_v2.select()
        
        # 3. Actions
        ttk.Label(left_col, text="ACTIONS", font=("Roboto", 14, "bold"), foreground="#3B8ED0").pack(anchor="w", padx=15, pady=(30, 5))
        
        self.btn_load = ttk.Button(left_col, text="1. LOAD FILE", command=self._load_file, bootstyle="primary",  foreground="black")
        self.btn_load.pack(fill="x", padx=15, pady=5)
        
        self.btn_start = ttk.Button(left_col, text="2. Bắt đầu tạo voice", command=self._start, height=40, font=("Roboto", 13, "bold"))
        self.btn_start.pack(fill="x", padx=15, pady=5)
        
        self.btn_stop = ttk.Button(left_col, text="STOP", command=self._stop, bootstyle="primary",  state="disabled")
        self.btn_stop.pack(fill="x", padx=15, pady=5)

        # Progress
        self.progress_bar = ttk.Progressbar(left_col)
        self.progress_bar.set(0)
        self.progress_bar.pack(fill="x", padx=15, pady=(20, 5))
        
        self.lbl_status = ttk.Label(left_col, text="Ready", foreground="gray")
        self.lbl_status.pack(padx=15)

        # --- RIGHT MAIN AREA ---
        right_col = ttk.Frame(main_paned, bootstyle="transparent")
        right_col.pack(side="left", fill="both", expand=True)
        right_col.grid_rowconfigure(1, weight=1)
        right_col.grid_columnconfigure(0, weight=1)

        # Top: Audio Preview List (Custom Scrollable Frame)
        preview_frame = ttk.Frame(right_col, bootstyle="primary", )
        preview_frame.grid(row=0, column=0, sticky="nsew", pady=(0, 10))
        
        header_frame = ttk.Frame(preview_frame, height=40, bootstyle="primary")
        header_frame.pack(fill="x", padx=2, pady=2)
        ttk.Label(header_frame, text="Generated Audio Queue", font=("Roboto", 13, "bold")).pack(side="left", padx=10)
        
        ttk.Button(header_frame, text="Clear List", width=80, height=25, bootstyle="primary", command=self._clear_audio_list).pack(side="right", padx=5)
        ttk.Button(header_frame, text="Play All", width=80, height=25, bootstyle="green", command=self._play_all).pack(side="right", padx=5)
        ttk.Button(header_frame, text="Stop Audio", width=80, height=25, bootstyle="red", command=self._stop_playback).pack(side="right", padx=5)
        
        # Scrollable container for rows
        self.audio_scroll = ScrolledFrame(preview_frame, label_text="Audio Files")
        self.audio_scroll.pack(fill="both", expand=True, padx=5, pady=5)

        # Bottom: Logs
        log_frame = ttk.Frame(right_col, height=200, bootstyle="primary")
        log_frame.grid(row=1, column=0, sticky="nsew")
        
        ttk.Label(log_frame, text="System Log", font=("Consolas", 12)).pack(anchor="w", padx=5, pady=2)
        self.txt_log = ScrolledText(log_frame, font=("Consolas", 11), state="disabled", foreground="#00FF00")
    ...
    def _setup_longtext_tab(self):
        ...
        self.lt_combo_voice = ttk.Combobox(ctrl_frame, values=VOICES, width=150)
        self.lt_combo_voice.set(self.voice_val)
        self.lt_combo_voice.pack(side="left", padx=10, pady=10)
        
        # File Select
        self.btn_lt_files = ttk.Button(ctrl_frame, text="Select .txt Files", command=self._lt_browse_files)
        self.btn_lt_files.pack(side="left", padx=10)
        
        self.lbl_lt_files = ttk.Label(ctrl_frame, text="No files selected", foreground="gray")
        self.lbl_lt_files.pack(side="left", padx=5)

        # Main Split
        content_frame = ttk.Frame(main_paned, bootstyle="transparent")
        content_frame.pack(fill="both", expand=True, pady=5)
        content_frame.grid_columnconfigure(0, weight=1)
        content_frame.grid_columnconfigure(1, weight=1)

        # Left: Direct Text
        left_f = ttk.Frame(content_frame)
        left_f.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        
        ttk.Label(left_f, text="DIRECT TEXT INPUT", font=("Roboto", 14, "bold")).pack(pady=10)
        self.lt_txt_input = ScrolledText(left_f, wrap="word")
        self.lt_txt_input.pack(fill="both", expand=True, padx=10, pady=5)
        
        self.lt_lbl_chars = ttk.Label(left_f, text="Chars: 0")
        self.lt_lbl_chars.pack(pady=5)
        self.lt_txt_input.bind("<KeyRelease>", self._lt_update_char_count)

        self.btn_lt_direct = ttk.Button(left_f, text="PROCESS THIS TEXT", bootstyle="green", command=self._lt_process_direct_text)
        self.btn_lt_direct.pack(fill="x", padx=10, pady=10)

        # Right: Config & Logs
        right_f = ttk.Frame(content_frame)
        right_f.grid(row=0, column=1, sticky="nsew", padx=(5, 0))
        
        # Configs
        cfg_box = ttk.Frame(right_f)
        cfg_box.pack(fill="x", padx=10, pady=10)
        
        self.lt_entry_out = ttk.Entry(cfg_box, placeholder_text="Output Dir")
        self.lt_entry_out.insert(0, "./long_text_output")
        self.lt_entry_out.pack(fill="x", padx=5, pady=5)
        
        # FFmpeg path - use from settings
        ffmpeg_frame = ttk.Frame(cfg_box, bootstyle="transparent")
        ffmpeg_frame.pack(fill="x", padx=5, pady=5)
        
        ttk.Label(ffmpeg_frame, text="FFmpeg:", width=60).pack(side="left")
        self.lt_entry_ffmpeg = ttk.Entry(ffmpeg_frame, placeholder_text="Path to ffmpeg.exe")
        ...
        chunk_frame = ttk.Frame(cfg_box, bootstyle="transparent")
        chunk_frame.pack(fill="x", padx=5, pady=5)
        
        ttk.Label(chunk_frame, text="Chunk size:", width=80).pack(side="left")
        self.lt_chunk_size = ttk.Entry(chunk_frame, placeholder_text="Chunk size (300)", width=80)
        self.lt_chunk_size.insert(0, "300")
        self.lt_chunk_size.pack(side="left", padx=(5, 10))
        
        # Ngắt dòng v2 checkbox
        self.lt_sw_chunk_v2 = ttk.Checkbutton(chunk_frame, text="Ngắt dòng v2 (theo dấu câu)")
        self.lt_sw_chunk_v2.pack(side="left", padx=5)
        if self.lt_chunk_v2_enabled:
            self.lt_sw_chunk_v2.select()

        self.lt_sw_del = ttk.Checkbutton(cfg_box, text="Delete temp chunks")
        self.lt_sw_del.select()
        self.lt_sw_del.pack(anchor="w", padx=5, pady=5)
        
        # Live Session Mode checkbox - phiên làm việc liên tục như cuộc gọi điện thoại
        self.lt_sw_live_session = ttk.Checkbutton(cfg_box, text="Session Mode)")
        self.lt_sw_live_session.pack(anchor="w", padx=5, pady=5)
        
        # Keep Voice Beta Mode - giữ giọng ổn định bằng cách wrap text trong {content}
        self.lt_sw_keep_voice_beta = ttk.Checkbutton(cfg_box, text="Giữ giọng beta")
        self.lt_sw_keep_voice_beta.pack(anchor="w", padx=5, pady=5)

        self.btn_lt_process_files = ttk.Button(cfg_box, text="PROCESS SELECTED FILES", bootstyle="primary", foreground="black", command=self._lt_process_files)
        self.btn_lt_process_files.pack(fill="x", padx=5, pady=10)
        
        self.btn_lt_stop = ttk.Button(cfg_box, text="STOP", bootstyle="red", state="disabled", command=self._lt_stop)
        self.btn_lt_stop.pack(fill="x", padx=5, pady=5)

        # Log
        ttk.Label(right_f, text="Processing Log").pack()
        self.lt_txt_log = ScrolledText(right_f, state="disabled", foreground="#E09F3E", font=("Consolas", 10))
        self.lt_txt_log.pack(fill="both", expand=True, padx=10, pady=10)

        self.lt_progress = ttk.Progressbar(right_f)
    ...
    def _setup_multivoice_tab(self):
        ...
        raw_frame = ttk.Frame(tab, bootstyle="primary", )
        raw_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 2), pady=0)
       
        
        raw_text_content = """Đang phát triển"""
        
        txt_raw = ScrolledText(raw_frame, font=("Consolas", 11), bootstyle="primary", foreground="#aaa")
        ...
        builder_frame = ttk.Frame(tab, bootstyle="transparent")
        builder_frame.grid(row=0, column=1, sticky="nsew", padx=5, pady=0)
        
        ttk.Label(builder_frame, text="📝 Trình tạo kịch bản", font=("Roboto", 16, "bold")).pack(anchor="w", pady=(10, 5))

        # Style Instructions
        ttk.Label(builder_frame, text="Chỉ dẫn phong cách (Style instructions)").pack(anchor="w", pady=(5, 2))
        style_entry = ttk.Entry(builder_frame, placeholder_text="Ví dụ: Đọc to với giọng ấm áp, chào đón", height=35)
        style_entry.insert(0, "Read aloud in a warm, welcoming tone")
        style_entry.pack(fill="x", pady=(0, 15))

        # Scrollable Area for Dialogs
        self.dialog_scroll = ScrolledFrame(builder_frame, label_text="Hội thoại (Dialogs)", bootstyle="primary")
        ...
        def create_dialog_block(parent, speaker_idx, default_text=""):
            block = ttk.Frame(parent, bootstyle="transparent")
            ...
            badge_frame = ttk.Frame(block, bootstyle="transparent")
            badge_frame.pack(anchor="w", pady=(0, 5))
            
            # Dot indicator
            dot = ttk.Label(badge_frame, text="●", font=("Arial", 16), foreground=color)
            dot.pack(side="left", padx=(0, 5))
            
            # Label name
            lbl = ttk.Label(badge_frame, text=f"Người nói {speaker_idx}", font=("Roboto", 12, "bold"), foreground="#ddd")
            lbl.pack(side="left")

            # Text Input
            inp = ScrolledText(block, height=60, autohide=True, bootstyle="primary")
            inp.pack(fill="x")
            if default_text:
                inp.insert("1.0", default_text)
            else:
                inp.insert("1.0", "Nhập nội dung hội thoại tại đây...")
                inp.configure(foreground="gray")
        ...
        # Add Dialog Button
        btn_add = ttk.Button(builder_frame, text="⊕ Thêm hội thoại", bootstyle="primary",  height=35)
        btn_add.pack(fill="x", pady=5)

        # --- CỘT 3: CÀI ĐẶT GIỌNG ĐỌC (Voice settings) ---
        settings_frame = ttk.Frame(tab, bootstyle="primary", )
        settings_frame.grid(row=0, column=2, sticky="nsew", padx=(2, 0), pady=0)

        ttk.Label(settings_frame, text="⚙️ Cài đặt giọng đọc", font=("Roboto", 14, "bold")).pack(anchor="w", padx=15, pady=(15, 10))

        # Scrollable Settings
        setting_scroll = ScrolledFrame(settings_frame, bootstyle="transparent")
        ...
        def create_speaker_setting(parent, speaker_idx, default_voice):
            ...
            card = ttk.Frame(parent, bootstyle="primary")
            card.pack(fill="x", pady=10)

            # Header
            header = ttk.Frame(card, bootstyle="transparent")
            header.pack(fill="x", padx=10, pady=5)
            
            ttk.Label(header, text="●", foreground=color).pack(side="left")
            ttk.Label(header, text=f"Cài đặt Người nói {speaker_idx}", font=("Roboto", 12, "bold")).pack(side="left", padx=5)
            ttk.Label(header, text="^", foreground="gray").pack(side="right") # Fake collapse icon

            # Content
            content = ttk.Frame(card, bootstyle="transparent")
            content.pack(fill="x", padx=10, pady=(0, 10))

            # Name Input
            ttk.Label(content, text="Tên hiển thị", font=("Roboto", 11), foreground="gray").pack(anchor="w")
            entry_name = ttk.Entry(content, height=30)
            entry_name.insert(0, f"Speaker {speaker_idx}")
            entry_name.pack(fill="x", pady=(2, 10))

            # Voice Select
            ttk.Label(content, text="Giọng đọc (Voice)", font=("Roboto", 11), foreground="gray").pack(anchor="w")
            combo = ttk.Combobox(content, values=VOICES, height=30)
        ...
        # Info note
        ttk.Label(settings_frame, text="*Tính năng UI Demo, chưa hoạt động.", foreground="gray", font=("Arial", 10)).pack(side="bottom", pady=10)
    ...
    def _setup_capcut_tab(self):
        ...
        # --- LEFT: Controls ---
        left_frame = ttk.Frame(tab, bootstyle="primary")
        left_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5), pady=0)

        # Header
        header = ttk.Frame(left_frame, bootstyle="primary", height=45)
        header.pack(fill="x")
        ttk.Label(header, text="🎤 CAPCUT VOICE TTS", font=("Roboto", 14, "bold"), foreground="white").pack(pady=10)

        # Session ID display
        session_frame = ttk.Frame(left_frame, bootstyle="transparent")
        session_frame.pack(fill="x", padx=15, pady=10)
        
        ttk.Label(session_frame, text="Session ID:", font=("Roboto", 11, "bold"), foreground="#f87171").pack(anchor="w")
        self.capcut_session_display = ttk.Label(session_frame, text="Chưa cài đặt", foreground="gray", font=("Consolas", 10))
        self.capcut_session_display.pack(anchor="w")
        self._update_capcut_session_display()

        # Filter section
        ttk.Label(left_frame, text="BỘ LỌC GIỌNG", font=("Roboto", 12, "bold"), foreground="#f87171").pack(anchor="w", padx=15, pady=(20, 5))
        
        filter_frame = ttk.Frame(left_frame, bootstyle="transparent")
        filter_frame.pack(fill="x", padx=15, pady=5)

        # --- SỬA ĐỔI: Thay ComboBox Ngôn ngữ bằng Scrollable Frame ---
        ttk.Label(filter_frame, text="Ngôn ngữ:", font=("Roboto", 10)).pack(anchor="w")
        
        # Biến lưu giá trị ngôn ngữ được chọn
        self.capcut_lang_var = tk.StringVar(value="Tất cả")
        
        # Tạo khung cuộn cho danh sách ngôn ngữ (Chiều cao cố định 120px)
        self.capcut_lang_scroll = ScrolledFrame(filter_frame, height=120, bootstyle="primary")
        ...
        ttk.Label(filter_frame, text="Giới tính:", font=("Roboto", 10)).pack(anchor="w", pady=(10, 0))
        self.capcut_gender_filter = ttk.Combobox(filter_frame, values=CAPCUT_GENDERS, command=self._filter_capcut_voices)
        self.capcut_gender_filter.set("Tất cả")
        self.capcut_gender_filter.pack(fill="x", pady=5)
        # -----------------------------------------------------------

        # Voice list
        ttk.Label(left_frame, text="DANH SÁCH GIỌNG", font=("Roboto", 12, "bold"), foreground="#f87171").pack(anchor="w", padx=15, pady=(20, 5))
        
        self.capcut_voice_list = ScrolledFrame(left_frame, bootstyle="primary")
        ...
        # --- RIGHT: TTS Interface ---
        right_frame = ttk.Frame(tab, bootstyle="transparent")
        right_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0), pady=0)
        right_frame.grid_rowconfigure(1, weight=1)
        right_frame.grid_columnconfigure(0, weight=1)

        # Input section - Text Input
        input_frame = ttk.Frame(right_frame, bootstyle="primary")
        input_frame.grid(row=0, column=0, sticky="nsew", pady=(0, 5))

        ttk.Label(input_frame, text="NHẬP VĂN BẢN", font=("Roboto", 14, "bold"), foreground="#f87171").pack(anchor="w", padx=15, pady=(15, 5))
        
        self.capcut_text_input = ScrolledText(input_frame, height=80, autohide=True, font=("Roboto", 12))
        ...
        btn_frame = ttk.Frame(input_frame, bootstyle="transparent")
        btn_frame.pack(fill="x", padx=15, pady=5)

        self.btn_capcut_generate = ttk.Button(btn_frame, text="🎙️ TẠO AUDIO", bootstyle="primary", font=("Roboto", 12, "bold"), command=self._capcut_generate)
        self.btn_capcut_generate.pack(side="left", padx=5)

        self.btn_capcut_play = ttk.Button(btn_frame, text="▶ PHÁT", bootstyle="primary", state="disabled", command=self._capcut_play)
        self.btn_capcut_play.pack(side="left", padx=5)

        self.btn_capcut_save = ttk.Button(btn_frame, text="💾 LƯU FILE", bootstyle="primary", state="disabled", command=self._capcut_save)
        self.btn_capcut_save.pack(side="left", padx=5)

        self.capcut_status_lbl = ttk.Label(btn_frame, text="Sẵn sàng", foreground="gray")
        self.capcut_status_lbl.pack(side="right", padx=10)

        # SRT/VTT Batch Processing Section
        srt_frame = ttk.Frame(right_frame, bootstyle="primary")
        srt_frame.grid(row=1, column=0, sticky="nsew", pady=5)

        ttk.Label(srt_frame, text="📝 TẠO VOICE TỪ FILE SRT/VTT HOẶC THƯ MỤC", font=("Roboto", 14, "bold"), foreground="#f87171").pack(anchor="w", padx=15, pady=(15, 5))

        # File selection
        file_row = ttk.Frame(srt_frame, bootstyle="transparent")
        file_row.pack(fill="x", padx=15, pady=5)

        self.capcut_srt_file_entry = ttk.Entry(file_row, placeholder_text="Chọn file SRT/VTT hoặc thư mục chứa txt/docx...", width=300)
        self.capcut_srt_file_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        ttk.Button(file_row, text="📂 File", width=60, command=self._capcut_browse_srt).pack(side="left", padx=2)
        ttk.Button(file_row, text="📁 Thư mục", width=80, command=self._capcut_browse_folder).pack(side="left", padx=2)

        # Output folder
        out_row = ttk.Frame(srt_frame, bootstyle="transparent")
        out_row.pack(fill="x", padx=15, pady=5)

        self.capcut_srt_output_entry = ttk.Entry(out_row, placeholder_text="Thư mục output...")
        ...
        ttk.Button(out_row, text="📁 Chọn", width=100, command=self._capcut_browse_output).pack(side="left")

        # Options row - merge option
        opt_row = ttk.Frame(srt_frame, bootstyle="transparent")
        opt_row.pack(fill="x", padx=15, pady=5)
        
        self.capcut_merge_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opt_row, text="Hợp nhất voice sau khi xong (Merge all)", 
                       variable=self.capcut_merge_var, 
                       font=("Roboto", 11)).pack(side="left", padx=5)

        # Process buttons
        proc_btn_row = ttk.Frame(srt_frame, bootstyle="transparent")
        proc_btn_row.pack(fill="x", padx=15, pady=10)

        self.btn_capcut_process_srt = ttk.Button(
        ...
        self.btn_capcut_stop_srt = ttk.Button(
        ...
        self.btn_capcut_preview = ttk.Button(
        ...
        # Progress
        self.capcut_srt_progress = ttk.Progressbar(srt_frame)
        self.capcut_srt_progress.set(0)
        self.capcut_srt_progress.pack(fill="x", padx=15, pady=5)

        self.capcut_srt_status = ttk.Label(srt_frame, text="Sẵn sàng xử lý", foreground="gray")
        self.capcut_srt_status.pack(anchor="w", padx=15, pady=(0, 10))

        # Log section
        log_frame = ttk.Frame(right_frame, bootstyle="primary")
        log_frame.grid(row=2, column=0, sticky="nsew")

        ttk.Label(log_frame, text="LOG", font=("Roboto", 12, "bold"), foreground="#f87171").pack(anchor="w", padx=15, pady=(10, 5))
        self.capcut_log = ScrolledText(log_frame, font=("Consolas", 10), foreground="#22c55e", state="disabled")
    ...
    def _setup_edge_tab(self):
        ...
        header = ttk.Frame(left_frame, bootstyle="primary", height=45)
        header.pack(fill="x")
        ttk.Label(header, text="🔊 EDGE TTS (Microsoft)", font=("Roboto", 14, "bold"), foreground="white").pack(pady=10)

        # Refresh button
        btn_refresh = ttk.Button(left_frame, text="🔄 Tải Voice List", bootstyle="primary", command=self._load_edge_voices)
        btn_refresh.pack(fill="x", padx=15, pady=10)

        self.edge_voice_count_lbl = ttk.Label(left_frame, text="Voices: 0", foreground="gray")
        self.edge_voice_count_lbl.pack(anchor="w", padx=15)

        # Filter section
        ttk.Label(left_frame, text="BỘ LỌC", font=("Roboto", 12, "bold"), foreground="#60a5fa").pack(anchor="w", padx=15, pady=(20, 5))
        ...
        self.edge_lang_var = tk.StringVar(value="Tất cả")
        
        # Tạo khung cuộn cho danh sách ngôn ngữ (Chiều cao cố định 150px vì danh sách này dài)
        self.edge_lang_scroll = ScrolledFrame(filter_frame, height=150, bootstyle="primary")
        ...
        ttk.Label(filter_frame, text="Giới tính:", font=("Roboto", 10)).pack(anchor="w", pady=(10, 0))
        self.edge_gender_filter = ttk.Combobox(filter_frame, values=EDGE_TTS_GENDERS, command=self._filter_edge_voices)
        self.edge_gender_filter.set("Tất cả")
        self.edge_gender_filter.pack(fill="x", pady=5)
        # -----------------------------------------------------------

        # Voice list
        ttk.Label(left_frame, text="DANH SÁCH GIỌNG", font=("Roboto", 12, "bold"), foreground="#60a5fa").pack(anchor="w", padx=15, pady=(20, 5))
        self.edge_voice_list = ScrolledFrame(left_frame, bootstyle="primary")
        ...
        ttk.Label(input_frame, text="NHẬP VĂN BẢN", font=("Roboto", 14, "bold"), foreground="#60a5fa").pack(anchor="w", padx=15, pady=(15, 5))
        
        self.edge_text_input = ScrolledText(input_frame, height=80, autohide=True, font=("Roboto", 12))
        self.edge_text_input.pack(fill="x", padx=15, pady=5)
        self.edge_text_input.insert("1.0", "Enter text to convert to speech...")

        # Voice settings
        settings_row = ttk.Frame(input_frame, bootstyle="transparent")
        settings_row.pack(fill="x", padx=15, pady=5)

        ttk.Label(settings_row, text="Rate:").pack(side="left")
        self.edge_rate = ttk.Entry(settings_row, width=70, placeholder_text="+0%")
        self.edge_rate.insert(0, "+0%")
        self.edge_rate.pack(side="left", padx=5)

        ttk.Label(settings_row, text="Volume:").pack(side="left", padx=(15, 0))
        self.edge_volume = ttk.Entry(settings_row, width=70, placeholder_text="+0%")
        self.edge_volume.insert(0, "+0%")
        self.edge_volume.pack(side="left", padx=5)

        ttk.Label(settings_row, text="Pitch:").pack(side="left", padx=(15, 0))
        self.edge_pitch = ttk.Entry(settings_row, width=70, placeholder_text="+0Hz")
        ...
        self.btn_edge_generate = ttk.Button(btn_frame, text="🎙️ TẠO AUDIO", bootstyle="primary", font=("Roboto", 12, "bold"), command=self._edge_generate)
        self.btn_edge_generate.pack(side="left", padx=5)

        self.btn_edge_play = ttk.Button(btn_frame, text="▶ PHÁT", bootstyle="primary", state="disabled", command=self._edge_play)
        self.btn_edge_play.pack(side="left", padx=5)

        self.btn_edge_save = ttk.Button(btn_frame, text="💾 LƯU FILE", bootstyle="primary", state="disabled", command=self._edge_save)
        self.btn_edge_save.pack(side="left", padx=5)

        self.edge_status_lbl = ttk.Label(btn_frame, text="Sẵn sàng", foreground="gray")
        ...
        ttk.Label(srt_frame, text="📝 TẠO VOICE TỪ FILE SRT/VTT HOẶC THƯ MỤC", font=("Roboto", 14, "bold"), foreground="#60a5fa").pack(anchor="w", padx=15, pady=(15, 5))

        # File selection
        file_row = ttk.Frame(srt_frame, bootstyle="transparent")
        file_row.pack(fill="x", padx=15, pady=5)

        self.edge_srt_file_entry = ttk.Entry(file_row, placeholder_text="Chọn file SRT/VTT hoặc thư mục...", width=300)
        self.edge_srt_file_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        ttk.Button(file_row, text="📂 File", width=60, command=self._edge_browse_srt).pack(side="left", padx=2)
        ttk.Button(file_row, text="📁 Thư mục", width=80, command=self._edge_browse_folder).pack(side="left", padx=2)

        # Output folder
        out_row = ttk.Frame(srt_frame, bootstyle="transparent")
        out_row.pack(fill="x", padx=15, pady=5)

        self.edge_srt_output_entry = ttk.Entry(out_row, placeholder_text="Thư mục output...")
        self.edge_srt_output_entry.insert(0, "./edge_output")
        self.edge_srt_output_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))

        ttk.Button(out_row, text="📁 Chọn", width=100, command=self._edge_browse_output).pack(side="left")

        # Options row
        opt_row = ttk.Frame(srt_frame, bootstyle="transparent")
        opt_row.pack(fill="x", padx=15, pady=5)

        ttk.Label(opt_row, text="Workers:").pack(side="left")
        self.edge_workers_var = tk.StringVar(value="10")
        ttk.Entry(opt_row, textvariable=self.edge_workers_var, width=60).pack(side="left", padx=5)
        
        self.edge_merge_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opt_row, text="Hợp nhất voice sau khi xong", 
        ...
        self.btn_edge_process_srt = ttk.Button(
            proc_btn_row, text="🚀 XỬ LÝ FILE/THƯ MỤC", 
            bootstyle="primary", 
            font=("Roboto", 12, "bold"), height=40,
        ...
        self.btn_edge_stop_srt = ttk.Button(
            proc_btn_row, text="⏹ DỪNG", 
            bootstyle="primary", state="disabled",
        ...
        self.btn_edge_preview = ttk.Button(
            proc_btn_row, text="🔊 Nghe thử", 
            bootstyle="primary",
        ...
        # Progress
        self.edge_srt_progress = ttk.Progressbar(srt_frame)
        self.edge_srt_progress.set(0)
        self.edge_srt_progress.pack(fill="x", padx=15, pady=5)

        self.edge_srt_status = ttk.Label(srt_frame, text="Sẵn sàng xử lý", foreground="gray")
        ...
        ttk.Label(log_frame, text="LOG", font=("Roboto", 12, "bold"), foreground="#60a5fa").pack(anchor="w", padx=15, pady=(10, 5))
        self.edge_log = ScrolledText(log_frame, font=("Consolas", 10), foreground="#22c55e", state="disabled")
    ...
    def _setup_vieneu_tab(self):
        ...
        header = ttk.Frame(left_frame, bootstyle="primary", height=45)
        header.pack(fill="x")
        ttk.Label(header, text="🇻🇳 VN TTS", font=("Roboto", 14, "bold"), foreground="white").pack(pady=10)
        
        # Scrollable content
        content_scroll = ScrolledFrame(left_frame, bootstyle="transparent")
        content_scroll.pack(fill="both", expand=True, padx=5, pady=5)

        # ===== MODEL CONFIGURATION SECTION =====
        ttk.Label(content_scroll, text="⚙️ CẤU HÌNH MODEL", font=("Roboto", 12, "bold"), foreground="#818cf8").pack(anchor="w", padx=10, pady=(10, 5))
        
        model_frame = ttk.Frame(content_scroll, bootstyle="primary")
        model_frame.pack(fill="x", padx=10, pady=5)
        
        # Backbone selection
        ttk.Label(model_frame, text="Backbone Model:", font=("Roboto", 10)).pack(anchor="w", padx=10, pady=(10, 2))
        self.vieneu_backbone_var = tk.StringVar(value="VN TTS Q4 (Nhanh)")
        self.vieneu_backbone_combo = ttk.Combobox(
            model_frame, 
        ...
        self.vieneu_backbone_desc = ttk.Label(
            model_frame, 
            text=VIENEU_BACKBONE_CONFIGS["VN TTS Q4 (Nhanh)"]["description"],
            font=("Roboto", 9), foreground="gray"
        )
        self.vieneu_backbone_desc.pack(anchor="w", padx=10, pady=2)
        
        # Codec selection
        ttk.Label(model_frame, text="Audio Codec:", font=("Roboto", 10)).pack(anchor="w", padx=10, pady=(10, 2))
        self.vieneu_codec_var = tk.StringVar(value="NeuCodec (Standard)")
        self.vieneu_codec_combo = ttk.Combobox(
            model_frame, 
        ...
        # Device selection
        ttk.Label(model_frame, text="Device:", font=("Roboto", 10)).pack(anchor="w", padx=10, pady=(10, 2))
        self.vieneu_device_var = tk.StringVar(value="Auto")
        device_frame = ttk.Frame(model_frame, bootstyle="transparent")
        device_frame.pack(fill="x", padx=10, pady=2)
        
        ttk.Radiobutton(device_frame, text="Auto", variable=self.vieneu_device_var, value="Auto").pack(side="left", padx=5)
        ttk.Radiobutton(device_frame, text="CPU", variable=self.vieneu_device_var, value="CPU").pack(side="left", padx=5)
        ttk.Radiobutton(device_frame, text="CUDA (GPU)", variable=self.vieneu_device_var, value="CUDA").pack(side="left", padx=5)
        
        # Advanced options
        adv_frame = ttk.Frame(model_frame, bootstyle="transparent")
        adv_frame.pack(fill="x", padx=10, pady=5)
        
        self.vieneu_triton_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(adv_frame, text="Enable Triton (GPU)", variable=self.vieneu_triton_var, font=("Roboto", 10)).pack(side="left", padx=5)
        
        ttk.Label(adv_frame, text="Max Batch:", font=("Roboto", 10)).pack(side="left", padx=(15, 5))
        self.vieneu_batch_var = tk.StringVar(value="8")
        ttk.Entry(adv_frame, textvariable=self.vieneu_batch_var, width=50).pack(side="left")
        
        # GPU Memory optimization slider
        gpu_opt_frame = ttk.Frame(model_frame, bootstyle="transparent")
        gpu_opt_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(gpu_opt_frame, text="GPU Memory:", font=("Roboto", 10)).pack(side="left", padx=5)
        # Slider from 30% to 80% GPU memory, steps of 10% (30, 40, 50, 60, 70, 80)
        self.vieneu_memory_slider = ttk.Scale(gpu_opt_frame, from_=0.3, to=0.8, number_of_steps=5, width=100)
        self.vieneu_memory_slider.set(0.5)  # Default to 50%
        self.vieneu_memory_slider.pack(side="left", padx=5)
        self.vieneu_memory_lbl = ttk.Label(gpu_opt_frame, text="50%", font=("Roboto", 9), width=35)
        ...
        # Load model button
        self.btn_vieneu_load = ttk.Button(
            model_frame, 
            text="🔄 TẢI MODEL", 
            bootstyle="primary", 
        ...
        # Model status
        self.vieneu_model_status = ttk.Label(
            model_frame, 
        ...
        ttk.Label(content_scroll, text="🎤 CHỌN GIỌNG NÓI", font=("Roboto", 12, "bold"), foreground="#818cf8").pack(anchor="w", padx=10, pady=(15, 5))
        
        voice_frame = ttk.Frame(content_scroll, bootstyle="primary")
        voice_frame.pack(fill="x", padx=10, pady=5)
        
        # Voice mode tabs
        self.vieneu_voice_mode = tk.StringVar(value="preset")
        mode_frame = ttk.Frame(voice_frame, bootstyle="transparent")
        ...
        # Preset voice list
        self.vieneu_preset_frame = ttk.Frame(voice_frame, bootstyle="transparent")
        self.vieneu_preset_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(self.vieneu_preset_frame, text="Chọn giọng:", font=("Roboto", 10)).pack(anchor="w")
        
        self.vieneu_voice_list = ScrolledFrame(self.vieneu_preset_frame, height=150, bootstyle="primary")
        self.vieneu_voice_list.pack(fill="x", pady=5)
        
        self.vieneu_selected_voice = tk.StringVar(value="Vĩnh (nam miền Nam)")
        self._vieneu_populate_voice_list()
        
        # Preview preset voice
        self.btn_vieneu_preview_preset = ttk.Button(
        ...
        self.vieneu_custom_frame = ttk.Frame(voice_frame, bootstyle="transparent")
        # Hidden by default
        
        ttk.Label(self.vieneu_custom_frame, text="📂 Upload file audio mẫu (.wav):", font=("Roboto", 10)).pack(anchor="w", pady=(5, 2))
        
        custom_audio_row = ttk.Frame(self.vieneu_custom_frame, bootstyle="transparent")
        custom_audio_row.pack(fill="x", pady=2)
        
        self.vieneu_custom_audio_entry = ttk.Entry(custom_audio_row, placeholder_text="Chọn file audio...")
        self.vieneu_custom_audio_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))
        
        ttk.Button(custom_audio_row, text="📂", width=40, command=self._vieneu_browse_custom_audio).pack(side="left")
        
        ttk.Label(self.vieneu_custom_frame, text="📝 Nội dung lời thoại mẫu:", font=("Roboto", 10)).pack(anchor="w", pady=(10, 2))
        self.vieneu_custom_text_input = ScrolledText(self.vieneu_custom_frame, height=80, autohide=True, font=("Roboto", 11))
        ...
        self.btn_vieneu_encode_custom = ttk.Button(
        ...
        save_voice_frame = ttk.Frame(self.vieneu_custom_frame, bootstyle="transparent")
        save_voice_frame.pack(fill="x", pady=(5, 2))
        
        ttk.Label(save_voice_frame, text="💾 Lưu giọng với tên:", font=("Roboto", 10)).pack(side="left", padx=(0, 5))
        self.vieneu_save_voice_name = ttk.Entry(save_voice_frame, placeholder_text="Tên giọng (VD: Hùng nam miền Bắc)", width=180)
        self.vieneu_save_voice_name.pack(side="left", fill="x", expand=True)
        
        self.btn_vieneu_save_voice = ttk.Button(
            self.vieneu_custom_frame,
        ...
        self.vieneu_custom_status = ttk.Label(
            self.vieneu_custom_frame,
            text="",
            font=("Roboto", 9),
            foreground="gray"
        ...
        self.btn_vieneu_clone_now = ttk.Button(
            self.vieneu_custom_frame,
        ...
        self.vieneu_clone_instruction = ttk.Label(
            self.vieneu_custom_frame,
        ...
        ttk.Label(input_frame, text="📝 NHẬP VĂN BẢN CẦN ĐỌC", font=("Roboto", 14, "bold"), foreground="#818cf8").pack(anchor="w", padx=15, pady=(15, 5))
        
        self.vieneu_text_input = ScrolledText(input_frame, height=120, autohide=True, font=("Roboto", 12))
        ...
        # Character count
        char_frame = ttk.Frame(input_frame, bootstyle="transparent")
        char_frame.pack(fill="x", padx=15)
        self.vieneu_char_count = ttk.Label(char_frame, text="Ký tự: 0", foreground="gray", font=("Roboto", 10))
        ...
        self.btn_vieneu_generate = ttk.Button(
            btn_frame, 
            text="🎙️ TẠO AUDIO", 
            bootstyle="primary", 
        ...
        self.btn_vieneu_play = ttk.Button(
            btn_frame, 
            text="▶ PHÁT", 
            bootstyle="primary", 
            state="disabled", 
        ...
        self.btn_vieneu_stop = ttk.Button(
            btn_frame, 
            text="⏹ DỪNG", 
            bootstyle="primary", 
        ...
        self.btn_vieneu_save = ttk.Button(
            btn_frame, 
            text="💾 LƯU FILE", 
            bootstyle="primary", 
            state="disabled", 
        ...
        self.vieneu_streaming_var = tk.BooleanVar(value=False)
        self.vieneu_streaming_cb = ttk.Checkbutton(
            btn_frame, 
            text="⚡ Streaming (GPU)", 
        ...
        self.vieneu_status_lbl = ttk.Label(btn_frame, text="Sẵn sàng", foreground="gray")
        self.vieneu_status_lbl.pack(side="right", padx=10)

        # File Processing Section
        file_frame = ttk.Frame(right_frame, bootstyle="primary")
        file_frame.grid(row=1, column=0, sticky="nsew", pady=5)

        ttk.Label(file_frame, text="📁 XỬ LÝ FILE HÀNG LOẠT", font=("Roboto", 14, "bold"), foreground="#818cf8").pack(anchor="w", padx=15, pady=(15, 5))

        # File selection
        file_row = ttk.Frame(file_frame, bootstyle="transparent")
        file_row.pack(fill="x", padx=15, pady=5)

        self.vieneu_file_entry = ttk.Entry(file_row, placeholder_text="Chọn file SRT/TXT/DOCX...")
        self.vieneu_file_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        ttk.Button(file_row, text="📂 Chọn file", width=100, command=self._vieneu_browse_file).pack(side="left", padx=2)
        ttk.Button(file_row, text="📁 Thư mục", width=100, command=self._vieneu_browse_folder).pack(side="left", padx=2)

        # Output folder
        out_row = ttk.Frame(file_frame, bootstyle="transparent")
        out_row.pack(fill="x", padx=15, pady=5)

        self.vieneu_output_entry = ttk.Entry(out_row, placeholder_text="Thư mục output...")
        self.vieneu_output_entry.insert(0, "./vieneu_output")
        self.vieneu_output_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))

        ttk.Button(out_row, text="📁 Chọn", width=100, command=self._vieneu_browse_output).pack(side="left")

        # Options
        opt_row = ttk.Frame(file_frame, bootstyle="transparent")
        opt_row.pack(fill="x", padx=15, pady=5)
        
        self.vieneu_merge_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(opt_row, text="Hợp nhất voice", variable=self.vieneu_merge_var, font=("Roboto", 11)).pack(side="left", padx=5)
        
        self.vieneu_delete_chunks_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(opt_row, text="Xóa chunks sau khi ghép", variable=self.vieneu_delete_chunks_var, font=("Roboto", 11)).pack(side="left", padx=15)

        # Process buttons
        proc_btn_row = ttk.Frame(file_frame, bootstyle="transparent")
        proc_btn_row.pack(fill="x", padx=15, pady=10)

        self.btn_vieneu_process = ttk.Button(
            proc_btn_row, 
            text="🚀 XỬ LÝ FILE", 
            bootstyle="primary", 
        ...
        self.btn_vieneu_stop_process = ttk.Button(
            proc_btn_row, 
            text="⏹ DỪNG", 
            bootstyle="primary", 
            state="disabled",
        ...
        # Progress
        self.vieneu_progress = ttk.Progressbar(file_frame)
        self.vieneu_progress.set(0)
        self.vieneu_progress.pack(fill="x", padx=15, pady=5)

        self.vieneu_file_status = ttk.Label(file_frame, text="Sẵn sàng xử lý", foreground="gray")
        ...
        ttk.Label(log_frame, text="LOG", font=("Roboto", 12, "bold"), foreground="#818cf8").pack(anchor="w", padx=15, pady=(10, 5))
        self.vieneu_log = ScrolledText(log_frame, font=("Consolas", 10), foreground="#22c55e", state="disabled")
    ...
    def _vieneu_load_model(self):
        ...
        self.vieneu_model_status.configure(text="⏳ VN TTS đang tải...", foreground="#fbbf24")
        ...
        def load_thread():
            try:
                ...
                self.after(0, lambda: self.vieneu_model_status.configure(text=status_msg, foreground="#22c55e"))
                ...
                self.after(0, lambda: self.vieneu_model_status.configure(text=f"❌ Lỗi import: {err_msg[:50]}", foreground="#ef4444"))
                self._vieneu_progress_log(0, "Lỗi tải")
            except Exception as e:
                err_msg = self._sanitize_error_message(str(e))
                self.after(0, lambda: self._vieneu_log(f"❌ Lỗi: {err_msg}"))
                self.after(0, lambda: self.vieneu_model_status.configure(text=f"❌ Lỗi: {err_msg[:50]}", foreground="#ef4444"))
                self._vieneu_progress_log(0, "Lỗi tải")
            finally:
    ...
    def _vieneu_encode_custom_voice(self):
        ...
        self.vieneu_custom_status.configure(text="⏳ Đang xử lý...", foreground="#fbbf24")
        ...
        def encode_thread():
            try:
                ...
                self.after(0, lambda: self.vieneu_custom_status.configure(text="✅ Sẵn sàng! Nhập văn bản và bấm nút bên dưới để tạo audio", foreground="#22c55e"))
                ...
                self.after(0, lambda: self.vieneu_custom_status.configure(text=f"❌ Lỗi: {str(e)[:30]}", foreground="#ef4444"))
    ...
    def _setup_script_tab(self):
        ...
        header = ttk.Frame(left_frame, bootstyle="primary", height=45)
        header.pack(fill="x")
        ttk.Label(header, text="📖 ĐỌC VOICE KỊCH BẢN", font=("Roboto", 14, "bold"), foreground="white").pack(pady=10)

        # TTS Engine Selection
        ttk.Label(left_frame, text="CHỌN ENGINE TTS", font=("Roboto", 12, "bold"), foreground="#a78bfa").pack(anchor="w", padx=15, pady=(20, 5))
        
        self.script_engine_var = tk.StringVar(value="edge")
        
        engine_frame = ttk.Frame(left_frame, bootstyle="primary")
        engine_frame.pack(fill="x", padx=15, pady=5)
        
        ttk.Radiobutton(engine_frame, text="Edge TTS (Microsoft)", 
                          variable=self.script_engine_var, value="edge",
                          command=self._script_on_engine_change).pack(anchor="w", pady=5, padx=10)
        ttk.Radiobutton(engine_frame, text="Capcut Voice (TikTok)", 
                          variable=self.script_engine_var, value="capcut",
                          command=self._script_on_engine_change).pack(anchor="w", pady=5, padx=10)
        ttk.Radiobutton(engine_frame, text="Google Gemini TTS", 
                          variable=self.script_engine_var, value="google",
                          command=self._script_on_engine_change).pack(anchor="w", pady=5, padx=10)

        # Language and Gender Filters for Script Tab
        ttk.Label(left_frame, text="BỘ LỌC GIỌNG", font=("Roboto", 12, "bold"), foreground="#a78bfa").pack(anchor="w", padx=15, pady=(15, 5))
        
        filter_frame = ttk.Frame(left_frame, bootstyle="primary")
        filter_frame.pack(fill="x", padx=15, pady=5)
        
        # Language filter
        ttk.Label(filter_frame, text="Ngôn ngữ:", font=("Roboto", 10)).pack(anchor="w", padx=10, pady=(10, 0))
        self.script_lang_var = tk.StringVar(value="Tất cả")
        
        # Create scrollable frame for language options
        self.script_lang_scroll = ScrolledFrame(filter_frame, height=80, bootstyle="primary")
        ...
        # Gender filter
        ttk.Label(filter_frame, text="Giới tính:", font=("Roboto", 10)).pack(anchor="w", padx=10, pady=(10, 0))
        self.script_gender_var = tk.StringVar(value="Tất cả")
        self.script_gender_filter = ttk.Combobox(
            filter_frame, 
        ...
        # Voice Selection
        ttk.Label(left_frame, text="CHỌN GIỌNG ĐỌC", font=("Roboto", 12, "bold"), foreground="#a78bfa").pack(anchor="w", padx=15, pady=(15, 5))
        
        self.script_voice_list = ScrolledFrame(left_frame, bootstyle="primary", height=200)
        self.script_voice_list.pack(fill="both", expand=True, padx=15, pady=5)
        
        self.script_selected_voice = tk.StringVar(value="en-US-EmmaMultilingualNeural")
        self._script_update_voice_list(update_lang_filters=False)

        # Processing Options
        ttk.Label(left_frame, text="TÙY CHỌN XỬ LÝ", font=("Roboto", 12, "bold"), foreground="#a78bfa").pack(anchor="w", padx=15, pady=(15, 5))
        
        options_frame = ttk.Frame(left_frame, bootstyle="transparent")
        options_frame.pack(fill="x", padx=15, pady=5)
        
        self.script_line_mode_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Mỗi dòng = 1 voice riêng", 
                       variable=self.script_line_mode_var,
                       font=("Roboto", 11)).pack(anchor="w", pady=2)
        
        self.script_merge_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(options_frame, text="Hợp nhất tất cả thành 1 file", 
                       variable=self.script_merge_var,
                       font=("Roboto", 11)).pack(anchor="w", pady=2)

        # Workers setting
        workers_row = ttk.Frame(options_frame, bootstyle="transparent")
        workers_row.pack(fill="x", pady=5)
        ttk.Label(workers_row, text="Workers:", font=("Roboto", 11)).pack(side="left")
        self.script_workers_var = tk.StringVar(value="5")
        ttk.Entry(workers_row, textvariable=self.script_workers_var, width=60).pack(side="left", padx=5)
        
        # Live Session Mode checkbox - chỉ hiển thị khi chọn Google Gemini
        self.script_live_session_var = tk.BooleanVar(value=False)
        self.script_live_session_chk = ttk.Checkbutton(
        ...
        ttk.Label(input_frame, text="📝 NHẬP VĂN BẢN KỊCH BẢN", font=("Roboto", 14, "bold"), foreground="#a78bfa").pack(anchor="w", padx=15, pady=(15, 5))
        ttk.Label(input_frame, text="Mỗi dòng văn bản sẽ tạo thành 1 file voice riêng biệt", font=("Roboto", 11), foreground="gray").pack(anchor="w", padx=15)

        # Text input area
        self.script_text_input = ScrolledText(input_frame, height=100, autohide=True, font=("Roboto", 12))
        ...
        file_row = ttk.Frame(input_frame, bootstyle="transparent")
        file_row.pack(fill="x", padx=15, pady=5)

        self.script_file_entry = ttk.Entry(file_row, placeholder_text="Hoặc chọn file txt/docx hoặc thư mục...", width=300)
        self.script_file_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        ttk.Button(file_row, text="📂 File", width=60, command=self._script_browse_file).pack(side="left", padx=2)
        ttk.Button(file_row, text="📁 Thư mục", width=80, command=self._script_browse_folder).pack(side="left", padx=2)

        # Output folder
        out_row = ttk.Frame(input_frame, bootstyle="transparent")
        out_row.pack(fill="x", padx=15, pady=5)

        self.script_output_entry = ttk.Entry(out_row, placeholder_text="Thư mục output...")
        self.script_output_entry.insert(0, "./script_output")
        self.script_output_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))

        ttk.Button(out_row, text="📁 Chọn", width=100, command=self._script_browse_output).pack(side="left")

        # Process buttons
        proc_btn_row = ttk.Frame(input_frame, bootstyle="transparent")
        proc_btn_row.pack(fill="x", padx=15, pady=10)

        self.btn_script_process = ttk.Button(
        ...
        self.btn_script_stop = ttk.Button(
            proc_btn_row, text="⏹ DỪNG", 
            bootstyle="primary", state="disabled",
        ...
        # Nút ghép all voice
        self.btn_script_merge_all = ttk.Button(
        ...
        # Progress
        self.script_progress = ttk.Progressbar(input_frame)
        self.script_progress.set(0)
        self.script_progress.pack(fill="x", padx=15, pady=5)

        self.script_status = ttk.Label(input_frame, text="Sẵn sàng xử lý", foreground="gray")
        self.script_status.pack(anchor="w", padx=15, pady=(0, 10))

        # Generated Voice Preview Section
        preview_frame = ttk.Frame(right_frame, bootstyle="primary")
        preview_frame.grid(row=1, column=0, sticky="nsew", pady=5)

        header_preview = ttk.Frame(preview_frame, bootstyle="primary")
        header_preview.pack(fill="x")
        ttk.Label(header_preview, text="🔊 VOICE ĐÃ TẠO - NGHE THỬ", font=("Roboto", 12, "bold")).pack(side="left", padx=15, pady=8)
        
        ttk.Button(header_preview, text="🔄 Làm mới", width=80, height=28, 
                     command=self._script_refresh_preview).pack(side="right", padx=5, pady=5)
        ttk.Button(header_preview, text="⏹ Dừng phát", width=80, height=28, bootstyle="primary",
                     command=self.player.stop).pack(side="right", padx=5, pady=5)

        self.script_preview_list = ScrolledFrame(preview_frame, bootstyle="primary", height=150)
        ...
        ttk.Label(log_frame, text="LOG", font=("Roboto", 12, "bold"), foreground="#a78bfa").pack(anchor="w", padx=15, pady=(10, 5))
        self.script_log = ScrolledText(log_frame, font=("Consolas", 10), foreground="#22c55e", state="disabled")
    ...
    def _script_update_voice_list(self, update_lang_filters=False):
        ...
        if engine == "edge":
            ...
            if not voices:
                ttk.Label(self.script_voice_list, text="Đang tải voice list...", foreground="gray").pack(pady=10)
    ...
    def _script_refresh_preview(self):
        ...
        if not output_dir or not os.path.isdir(output_dir):
            ttk.Label(self.script_preview_list, text="Chưa có file audio", foreground="gray").pack(pady=10)
            return
        ...
        for audio_file in audio_files:
            ...
            row = ttk.Frame(self.script_preview_list, bootstyle="primary")
            row.pack(fill="x", pady=2, padx=5)
            
            ttk.Label(row, text=f"🎵 {file_name}", font=("Roboto", 10)).pack(side="left", padx=10, pady=5)
            ttk.Label(row, text=f"{file_size} KB", foreground="gray", font=("Roboto", 9)).pack(side="left", padx=5)
            
            ttk.Button(
                row, text="▶", width=35, height=25, bootstyle="primary",
    ...
    def _script_process(self):
        ...
        if engine == "capcut":
            ...
            if not session_id:
                ...
                # TODO: Fix - use tab object instead of name: self.notebook.select(self.tab_settings)
                return
        elif engine == "google":
    ...
    def _capcut_preview_files(self):
        ...
        ttk.Label(preview_win, text=f"📁 {output_dir}", font=("Roboto", 12), foreground="gray").pack(pady=5)
        ttk.Label(preview_win, text=f"Tìm thấy {len(mp3_files)} file audio", font=("Roboto", 14, "bold")).pack(pady=5)
        
        scroll_frame = ScrolledFrame(preview_win, bootstyle="primary")
        ...
        for mp3_file in mp3_files:
            ...
            row = ttk.Frame(scroll_frame, bootstyle="primary")
            row.pack(fill="x", pady=2, padx=5)
            
            ttk.Label(row, text=f"🎵 {file_name}", font=("Roboto", 11)).pack(side="left", padx=10, pady=8)
            ttk.Label(row, text=f"{file_size} KB", foreground="gray", font=("Roboto", 10)).pack(side="left", padx=5)
            
            ttk.Button(
                row, text="▶", width=40, bootstyle="primary",
                command=lambda f=mp3_file: self.player.play(f)
            ).pack(side="right", padx=5, pady=5)
        
        # Stop button
        ttk.Button(preview_win, text="⏹ Dừng phát", bootstyle="primary", 
    ...
    def _refresh_custom_voice_list(self):
        ...
        if not self.capcut_custom_voices:
            ttk.Label(self.capcut_custom_list, text="Chưa có voice tùy chỉnh", foreground="gray").pack(pady=10)
            return
        
        for voice in self.capcut_custom_voices:
            row = ttk.Frame(self.capcut_custom_list, bootstyle="primary")
            row.pack(fill="x", pady=2)
            
            ttk.Label(row, text=f"{voice.get('display_name')} ({voice.get('voice_id')})", font=("Roboto", 10)).pack(side="left", padx=5, pady=5)
            
            btn_del = ttk.Button(row, text="✕", width=25, height=25, bootstyle="primary", 
    ...
    def _add_audio_row(self, audio: GeneratedAudio):
        ...
        row = ttk.Frame(self.audio_scroll, bootstyle="primary")
        row.pack(fill="x", pady=2, padx=2)
        
        # Info
        ttk.Label(row, text=f"#{audio.index:04d}", width=50, font=("Consolas", 12, "bold")).pack(side="left", padx=5)
        ttk.Label(row, text=f"{audio.duration_ms:.0f}ms", width=60, foreground="gray").pack(side="left", padx=5)
        
        # Text Preview
        preview = audio.text[:60] + "..." if len(audio.text) > 60 else audio.text
        ttk.Label(row, text=preview, anchor="w").pack(side="left", fill="x", expand=True, padx=5)
        
        # Play Button
        btn = ttk.Button(row, text="▶", width=30, height=25, bootstyle="primary", 
                            command=lambda event=None: self.player.play(audio.file_path))
